...
CREATE OR REPLACE FUNCTION num_nodes(geom geometry)
RETURNS  int
AS $$
DECLARE
	BEGIN
		RETURN( SELECT count(crimen.idcrimen)   
			FROM crimen 
			WHERE st_within(st_geometryfromtext('POINT(' || longitud || ' ' || latitud || ')',4326), geom));
	END;
$$ LANGUAGE plpgsql;
...
/**PRIMERO INSERTARAL SHPEFILE, EN LA BAS EDE DATOS*/
/*PRIMERO*/
select st_srid (geom)  from ayac
select updategeometrysrid ('ayac', 'geom', 4326);
select st_srid(geom) from ayac;

select ST_AsGeoJSON(geom),ST_AsText(geom),num_nodes(geom) from ayac;
select gid, num_nodes(geom) as cantidad from ayac;
